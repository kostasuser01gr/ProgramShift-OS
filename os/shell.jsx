...
const REGISTRY = [
  ...
  { id: 'employee',   icon: 'user',     tint: '#3f6b41', category: 'Schedule', perm: 'app.view',          status: 'live', comp: 'EmployeeMobileModule', name: { el: 'Εφαρμογή Υπαλλήλου', en: 'Employee App' }, desc: { el: 'Η εμπειρία κινητού.', en: 'The mobile experience.' } },
...
const COMP = {
  ...
  ImportExportModule, EmployeeMobileModule, MembersModule, SettingsModule,
  DailyBriefingModule, DataQualityModule, SystemHealthModule, FilesModule, FormBuilderModule
};
...
function AIDrawer({ onClose }) {
  ...
  const [msgs, setMsgs] = React.useState([{ who: 'bot', text: en ? 'Hi — I can see ' + ctx.month.label.en + ' and your role. Ask me anything about the schedule.' : 'Γεια — βλέπω τον ' + ctx.month.label.el + ' και τον ρόλο σου. Ρώτησέ με για το πρόγραμμα.' }]);
  const [q, setQ] = React.useState('');
  const [busy, setBusy] = React.useState(false);
  const bodyRef = React.useRef(null);
  const suggestions = en ? ['Summarize coverage', 'What are the problems?', 'Who has the most nights?', 'Is the workload fair?'] : ['Σύνοψη κάλυψης', 'Ποια είναι τα προβλήματα;', 'Ποιος έχει πιο πολλές νύχτες;', 'Είναι δίκαιος ο φόρτος;'];
  ...
  async function ask(text) {
    ...
    setMsgs(function (m) { return m.concat([{ who: 'bot', text: answer }]); }); setBusy(false);
    setTimeout(function () { if (bodyRef.current) bodyRef.current.scrollTop = bodyRef.current.scrollHeight; }, 30);
  }
    ...
    <React.Fragment>
      ...
      <div className="drawer">
        <div className="dr-head">
          ...
          <div style={{ flex: 1 }}><div className="t">{en ? 'AI Assistant' : 'Βοηθός AI'}</div><div className="s">{OS.AI.available() ? (en ? 'Live · sees ' + ctx.month.label.en : 'Live · βλέπει ' + ctx.month.label.el) : (en ? 'Local · sees ' + ctx.month.label.en : 'Τοπικό · βλέπει ' + ctx.month.label.el)}</div></div>
        ...
        <div className="dr-foot">
          <div className="ai-input">
            <input value={q} onChange={function (e) { setQ(e.target.value); }} onKeyDown={function (e) { if (e.key === 'Enter') ask(); }} placeholder={en ? 'Ask about the schedule…' : 'Ρώτησε για το πρόγραμμα…'} />
