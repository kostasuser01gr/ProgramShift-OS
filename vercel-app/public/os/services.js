...
(function () {
  ...
  function save(key, val) {
    try { localStorage.setItem('psos.' + key, JSON.stringify(val)); } catch { /* quota / private mode */ }
  }
  ...
  var AI = {
    ...
    complete: async function (prompt) {
      if (window.claude && window.claude.complete) {
        try { return await window.claude.complete(prompt); } catch { return null; }
      }
      return null;
    }
  };
    ...
    function seed() {
      ...
      emps.forEach(function (e) {
        list.push({ id: 'm_' + e.ame, name: e.name, first: e.first, surname: e.surname,
          ...
          role: 'employee', dept: null, ame: e.ame });
      });
      return list;
    }
    var items = load('members', null);
    var liveIds = seed().map(function (member) { return member.ame; });
    if (items == null || !items.length || items.some(function (member) { return liveIds.indexOf(member.ame) < 0; })) {
      items = seed();
      save('members', items);
    }
  ...
  // ---- auth / session --------------------------------------------------------
  // NextAuth is authoritative in production. Name recognition remains available
  // only to local roster tools and never grants a production role.
  ...
  var Auth = (function () {
    var session = null;
    try {
      localStorage.removeItem('psos.creds');
      localStorage.removeItem('psos.savedlogins');
      localStorage.removeItem('psos.session');
    } catch { /* storage unavailable */ }
    return {
      current: function () { return session; },
      setExternal: function (value) {
        var role = ['owner', 'manager', 'employee'].indexOf(value.role) >= 0 ? value.role : 'employee';
        session = { id: value.id || value.email, name: value.name || value.email, email: value.email, role: role, image: value.image || null };
        Bus.emit('user.logged_in', session);
        Audit.add({ action: 'user.login.google', target: session.email, role: role, module: 'auth' });
        return session;
      },
      logout: function () {
        Audit.add({ action: 'user.logout', role: session && session.role, module: 'auth' });
        session = null;
        Bus.emit('user.logged_out', {});
        return window.ProgramShiftAPI.signOut();
      }
    };
  })();
