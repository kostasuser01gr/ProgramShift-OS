...
function OSRoot() {
  ...
  function updateLocalCell(ei, j, value, expected) {
    setEmpsMap(function (prev) {
      ...
      if (expected !== undefined && e.shifts[j] !== expected) return prev;
      const before = e.shifts[j];
      e.shifts[j] = value; e.cats[j] = D.category(value); cur[ei] = e;
      return Object.assign({}, prev, { [activeKey]: cur });
    });
  }
  async function setCell(ei, j, value) {
    const employee = emps[ei];
    const before = employee.shifts[j];
    updateLocalCell(ei, j, value);
    OS.Audit.add({ action: 'schedule.edit.pending', target: employee.name + ' · ' + month.dates[j].day + '/' + month.month, before: before, after: value, role: role, module: 'schedule' });
    if (month.source !== 'server') {
      OS.Notify.add({ type: 'system', title: lang === 'en' ? 'Local-only month' : 'Μόνο τοπικός μήνας', desc: lang === 'en' ? 'This edit is stored in this browser only.' : 'Η αλλαγή αποθηκεύεται μόνο σε αυτόν τον browser.', module: 'schedule' });
      return;
    }
    try {
      await window.ProgramShiftAPI.updateCell({ ame: employee.ame, dayIndex: j, value: value });
      OS.Audit.add({ action: 'schedule.edit.saved', target: employee.name + ' · ' + month.dates[j].day + '/' + month.month, before: before, after: value, role: role, module: 'schedule' });
      OS.Bus.emit('record.updated', { type: 'shift', emp: employee.ame, day: month.dates[j].day });
      OS.Memory.add({ kind: 'edit', text: (lang === 'en' ? 'Shift change · ' : 'Αλλαγή βάρδιας · ') + employee.name + ' ' + month.dates[j].day + '/' + month.month + ' → ' + value });
    } catch (error) {
      updateLocalCell(ei, j, before, value);
      OS.Audit.add({ action: 'schedule.edit.failed', target: employee.name, before: value, after: before, role: role, module: 'schedule' });
      OS.Notify.add({ type: 'warning', title: lang === 'en' ? 'Edit was not saved' : 'Η αλλαγή δεν αποθηκεύτηκε', desc: error.message, module: 'schedule', priority: 'high' });
      alert((lang === 'en' ? 'The edit was rolled back: ' : 'Η αλλαγή αναιρέθηκε: ') + error.message);
    }
  }
  ...
  async function logout() {
    setSession(null); setViewAs(null); setTabs(['launcher']); setOpenId('launcher');
    try { await OS.Auth.logout(); } catch (error) { window.location.assign('/api/auth/signout?callbackUrl=/'); }
  }
