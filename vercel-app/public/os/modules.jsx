...
function LauncherModule() {
  ...
  function Card(m) {
    ...
    const status = m.localOnly ? (lang === 'en' ? 'Local only' : 'Μόνο τοπικά') : (m.status === 'soon' ? (lang === 'en' ? 'Coming soon' : 'Σύντομα') : (lang === 'en' ? 'Live' : 'Ενεργό'));
    return (
      <div key={m.id} className="ln-card" onClick={function () { open(m.id); }}>
        ...
        <span className={'ln-chip ' + (m.status === 'soon' || m.localOnly ? 'soon' : 'live')}>{status}</span>
  ...
  return (
    <div className="ln-wrap">
      ...
      <div className="os-panel" style={{ padding: '10px 14px', marginBottom: 14, borderColor: 'var(--amber)' }}>
        <strong>{lang === 'en' ? 'Persistence:' : 'Αποθήκευση:'}</strong>{' '}
        {lang === 'en' ? 'Schedule modules use Google Sheets. Apps marked “Local only” store data in this browser and are not shared.' : 'Οι ενότητες προγράμματος χρησιμοποιούν Google Sheets. Οι εφαρμογές «Μόνο τοπικά» αποθηκεύουν δεδομένα μόνο σε αυτόν τον browser.'}
      </div>
