...
self.addEventListener('fetch', function (e) {
  ...
  if (url.origin !== self.location.origin || url.pathname.indexOf('/api/') === 0 || url.pathname === '/app/data.js') return;
  // Network-first: always try the live file, fall back to cache only when offline.
  e.respondWith(
    fetch(e.request).then(function (res) {
      if (!res.ok || res.redirected) return res;
      var copy = res.clone();
      caches.open(CACHE).then(function (c) { try { c.put(e.request, copy); } catch { } });
      return res;
    }).catch(function () {
